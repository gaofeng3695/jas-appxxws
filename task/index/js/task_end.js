...
/*跳转到详情页面的处置信息*/
/*function open_disposal(e){
    var taskId=$(e).closest(".event_list").find("input[name='taskId']").val();
    var eventId=$(e).closest(".event_list").find("input[name='eventId']").val();
    var params={
        "taskId":taskId,
        "eventId":eventId
    };
    appcan.locStorage.setVal("task_details_id",params );
    appcan.locStorage.setVal('page_num',1);
    appcan.openWinWithUrl("task_details","../task_details.html");
//    appcan.frame.evaluateScript("task_details","task_details_content",refresh(1));
//    uexWindow.evaluateMultiPopoverScript("", "task_details", "task_details_content", "alert('multipop')");
