...
tabview.on("click",function(obj, index){ /*TAB变更时切换多浮动窗口*/
    ...
    appcan.window.selectMultiPopover("taskAllListContent",index);
});
...
appcan.ready(function() {
    ...
    appcan.frame.open({/*创建多浮动窗口*/
        id : "content",
        url : [ {
            "inPageName" : "task_all_processing",//处理中
            "inUrl" : "task_list/task_all_processing.html"
        }, {
            "inPageName" : "task_all_end",
            "inUrl" : "task_list/task_all_end.html"
        }],
        top : titHeight,
        left : 0,
        index : 0,
        name : "taskAllListContent",
        change : function(index, res) {/*浮动窗口推拽变更时回调，可控制tab进行同步变更*/
            showNewsFooter(res.multiPopSelectedIndex);
            tabview.moveTo(res.multiPopSelectedIndex);
            if(numOpen==0){
                uexWindow.evaluateMultiPopoverScript("", "taskAllListContent", "task_all_end", "refreshList()");
                numOpen++;
            }
        }
    });
...
function refresh(eventStatus){
    if(eventStatus=="21"){
        appcan.window.selectMultiPopover("taskAllListContent",1);
        uexWindow.evaluateMultiPopoverScript("", "taskAllListContent", "task_all_end", "refreshList()");
    }else if(eventStatus=="20"){
        appcan.window.selectMultiPopover("taskAllListContent",0);
        uexWindow.evaluateMultiPopoverScript("", "taskAllListContent", "task_all_processing", "refreshList()");
    }
}
/*关闭任务删除处理中*/
function taskID_remover(e){
    uexWindow.evaluateMultiPopoverScript('','taskAllListContent','task_processing','taskID_remover("'+e+'")');
}