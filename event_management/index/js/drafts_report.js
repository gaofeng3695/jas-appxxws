...
var eventData = null;
var fullTypeName = null;
var selectTypeVal = null;
var num = 0;
var eventCodeData = null;
...
function initData() {
    ...
    event_id = appcan.locStorage.getVal("offlineEvent_Id");
    //uexLog.sendLog(eventId);
    appcan.locStorage.remove("offlineEvent_Id");
    var sql = "select * from eventlist where eventid='" + event_id + "';";
    dbOperation.dbSelect(sql, function(err, result) {
        if (err == null) {
            var resultData = result[0].postdata;
            eventData = JSON.parse(resultData);
            selectTypeVal = eventData.type + "_" + eventData.parentTypeId;
            
            //事件类型图标 sf 2017-08-14
            $("#eventIconName").val(eventData.eventIconName);
            
            //事件类型val
            $("#select_type").val(selectTypeVal);
            //formOperation.loadForm("#event_refer",eventData);
            fullTypeName = eventData.fullTypeName;
            //事件类型中文
            $(".text").text(fullTypeName);
            $("input[name='occurrenceTime']").val(eventData.occurrenceTime);
            //事件时间
            $("input[name='lon']").val(eventData.lon);
            //地图坐标
            $("input[name='lat']").val(eventData.lat);
            $("input[name='bdLon']").val(eventData.bdLon);
            $("input[name='bdLat']").val(eventData.bdLat);
            $("input[name='inspRecordId']").val(eventData.inspRecordId);
            //巡检唯一标示
            $("input[name='fullTypeName']").val(fullTypeName);
            $("input[name='address']").val(eventData.address);
            //地址中文
            $("textarea[name='description']").val(eventData.description);
            textareaLeng=$("textarea[name='description']").val().length;
            $("#word").text("("+(200-textareaLeng)+"字)");
            //内容
            //$("input[name='address']").val(eventData.address);

            eventCodeData = eventData.eventCode;
            var status=eventData.status;
            if(status=="30"){
                // $("#radio_no").checked=true;
                $(".pay_list_c1").removeClass("on");
                $('.whether_no').css("color",'#ff3f3f');
                $("input[name='radio']").each(function(e){
                    if($(this).val()=='30'){
                        $(this).prop("checked",true);
                        $(this).closest('.pay_list_c1').addClass('on');
                    }
                });
                var userBo = JSON.parse(appcan.locStorage.getVal("userBo"));
                ...
                /*初始化常用联系人*/
                people_data = eventData.taskUserMsg;
                var people_data1 = [];
                var people_dataList = null;
                var peopleNames = [];
                var userBo = JSON.parse(appcan.locStorage.getVal("userBo"));
                var myUserName = userBo.userName;
                for (var i = 0; i < people_data.length; i++) {
                    if (people_data[i].userName != myUserName) {
                        peopleNames.push(people_data[i].userName);
                        people_dataList = {
                            userId : people_data[i].userId,
                            userName : people_data[i].userName
                        }
                        people_data1.push(people_dataList);
                    }
                }
                var params = {
                    peo_data : people_data1,
                    peo_name : peopleNames
                };
                $(".whether_people").text(params.peo_name);
                appcan.locStorage.setVal("userSelect", params);
            }
            getAttaData(event_id);
        }
    });
}
    ...
    dbOperation.dbSelect(sql, function(err, result) {
        if (err == null) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                if (attaType == "pic") {
                    pic_scr += '<div class="ufl pic_list" id="preview_' + imageNumber + '">' + '<img src="' + data.postdata + '" ontouchstart="gtouchstart(this)" ontouchmove="gtouchmove(this)" ontouchend="gtouchend(this)" onclick="previewPicture(this)" height="260px" width="260px"  alt=""/>' + '</div>';
                    num++;
...
appcan.ready(function() {
    ...
    uexUploaderMgr.onStatus = function(opCode, fileSize, percent, serverPath, status) {
        ...
        case 2:
            ...
            appcan.window.evaluateScript('drafts_report', "changeButton()");
            break;
        }
    ...
    if (arr.length == 0) {//没有上传图片时，直接提交数据
        ...
        jasHttpRequest.jasHttpPost(partURL, function(id, state, dbSource) {
            ...
            if (resultData.success == "1") {
                deleteOfflineEventAttaData(event_id, function(resultDataB) {
                    if (resultDataB.success == "1") {
                        //baseOperation.alertToast("本地缓存数据清除成功");
                        deleteOfflineEventData(event_id, function(resultDataC) {
                            if (resultDataC.success == "1") {
                                sendZhugeMessager();
                            }
                        });
                    }
...
function sendZhugeMessager() {
    ...
    if (appcan.locStorage.getVal("enterEventModel") != "") {
        appcan.window.evaluateScript('daily_check', 'checkObj.showMap(true)');
    }
    appcan.window.evaluateScript('drafts_report', "utn_return('" + eventStatus + "')");
...
function on_submit(e) {
    ...
    //event_id =baseOperation.createuuid();  //生成唯一事件ID
    formArry.objectId = event_id;
    formArry.eventCode = eventCodeData;
...
function selectAddress() {
    var param = {
        "parentWindowId" : "drafts_report",
        "windowId" : "drafts_report_content",
        "callBackName" : "setPointValue()"
    };
    addressName=$("#address").val();
    bdLongitude=$("#lonBd").val();
    bdLatitude=$("#latBd").val();
    var addressInfo={
...
function loadEventType() {
    ...
    dbOperation.dbSelect(sql, function(err, data) {
        ...
        if (err != null) {
            ...
            for (var i in json_a) {
                txt += '<option value="' + json_a[i].objectId + '_' + json_a[i].parentId + '">' + json_a[i].typeName+'</option>';
            }
...
function saveOfflineData() {
    ...
    eventData.push(data);
    updateOfflineEventData(data, function(result) {
        if (result.success == "1") {
            ...
            if (isSaveOnLine) {
                ...
                appcan.window.evaluateScript('drafts_report', "utn_return('5')");
            }
        } else {
...
function saveOfflineAttaData(dataArry, businessId) {
    ...
    if (dataArry.length > 0) {
        ...
        deleteOfflineEventAttaData(businessId, function(resultData) {//删除附件信息
            if (resultData.success == "1") {
