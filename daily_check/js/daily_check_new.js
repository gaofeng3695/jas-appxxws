...
var MapObj = {
    ...
    addMarkersForFreeModel:function(_lat,_log,_zoomflag){
        var baiduPoint_free = MapObj.GPStoBaidu(_lat, _log); //获得百度坐标
        ...
        InitObj.readyFlag = 1;//此状态表示当前初始化已经完成，可以点击开始巡检
    },
    ...
    addPolylineOverlay:function(_data){
        ...
        if(pointsLength==0){
            ...
            this.lastPointForBaidu = {bdLon:item.bdLon,bdLat:item.bdLat};
...
var pipeLinesObj = {
    ...
    drowLines:function(){
        ...
        if(DomObj.isShowPipeLinesFlag==0){
            return;
        }
...
var LocationObj = {
    TIME_INTERVAL:5000,//默认GPS取点间隔 5秒
    openLocation:function(){
        uexLocation.openLocation("wgs84", this.TIME_INTERVAL); //开启定位       
    },
    closeLocation:function(){
        uexLocation.closeLocation();//关闭定位  
    },
    /*
     * 注册GPS坐标改变监听的事件
     */ 
    registerLocationChageEvent:function(){
        var zoomFlag = 1;//表示进行地图缩放（默认比例尺200m）
        uexLocation.onChange = function(lat, log, bRadio, bType, bSpeed) {
            if (lat < 10 && log < 10) {
                return;
            }
            //自由模式
            if (MapObj.modelStyle == 0) {
                MapObj.addMarkersForFreeModel(lat,log,zoomFlag);
                zoomFlag = 0;
                return;
            }
        }
    ...
    registerLocationChageEvent:function(){
        ...
        uexLockScreen.onChange = function(lat, lon, speed, distance,timer,satelliteNumber) {
            ...
            if(MapObj.lastPointForBaidu==null){
                ...
                MapObj.lastPointForBaidu = {bdLon:lon,bdLat:lat};
            }
            else{
                uexBaiduMap.addPolylineOverlay({
                    ...
                    fillColor: "#092af6",
                    lineWidth: 8,
                    property: [{longitude:MapObj.lastPointForBaidu.bdLon,latitude:MapObj.lastPointForBaidu.bdLat}, {longitude:lon,latitude:lat}],
                });
                _index++;
                MapObj.lastPointForBaidu = {bdLon:lon,bdLat:lat};
                /*
