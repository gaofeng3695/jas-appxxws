...
function initNewsData() {
    var objectId = appcan.locStorage.getVal("updateNews_param");
    ...
    dbOperation.dbSelect(sql, function(err, resultData) {
        ...
        $("#objcetId").val(thisData.objectId);
        $("#type").val(thisData.type);
        $("#title").val(thisData.title);
        $("#content").val(thisData.content);
        $("#people").val(thisData.receiverNames);
        textareaLeng=$("textarea[name='content']").val().length;
        $("#word").text("("+(160-textareaLeng)+"字)");
        listUser = thisData.listUser;
        initNewsAtta(objectId);
    });
}

function initNewsAtta(objectId) {
    var sql = "select * from localnewsatta where objectId='" + objectId + "'";
    dbOperation.dbSelect(sql, function(err, resultData) {
        if (!err) {
            for (var i = 0; i < resultData.length; i++) {
                var attaData = resultData[i];
                var mun = parseInt($(".img_mun").text());
                $(".img_mun").text(mun + 1);
                var option = '<div class="ufl" id="preview_' + imagesNumber + '">' + '<img src="' + attaData.postdata + '" ontouchstart="gtouchstart(this)"  ontouchend="gtouchend(this)" onclick="previewPicture(this)" height="260px" width="260px"  alt=""/>' + '</div>';
                $("#camera_img").append(option);
                imagesNumber++;
            }
        }
...
function openUserInfo() {
    var windowParam = {
        "parentWidowId" : "updateNews",
        "windowId" : "updatenews_content",
...
function checkForm() {
    ...
    if (submitNewsData.title == "") {
        ...
        appcan.window.evaluateScript('updateNews', "changeButtonStatus()");
        return;
    }
    ...
    var objectId=submitNewsData.objectId;
...
function saveOfflineAttaData(objectId) {
    try {
        ...
        if (imagesArray.length > 0) {
            deleteOfflineNewsAtta(objectId, function(resultA) {//删除消息相关的附件
                if (resultA.success == "1") {
                    for (var i = 0; i < imagesArray.length; i++) {
                        ...
                        var obj = {
                            "localId" : localid,
                            "objectId" : objectId,
                            "attaType" : imagesArray[i].type,
                            "localUrl" : url,
                            "postData" : imagesArray[i].path,
                            "state" : "0"
                        };
                        arrayData.push(obj);
                    }
...
function savaOfflineData() {
    try {
        ...
        newsData.push(data);
        //修改本地消息
        updateOfflineNews(data, function(result) {
            if (result.success == "1") {
                baseOperation.alertToast(result.msg);
                if (isSaveOnLine) {
                    subimtData();
                } else {
                    baseOperation.alertToast("数据修改完成！");
                    appcan.window.evaluateScript('updateNews', "closeWindow('5')");
                }
            } else {
                baseOperation.alertToast("数据修改失败！");
                appcan.window.evaluateScript('updateNews', "changeButtonStatus()");
            }
        });
    } catch(e) {
...
function saveData() {
    ...
    jasHttpRequest.jasHttpPost(partURL, function(id, state, dbSource) {
        ...
        if (resultData.success == "1") {
            ...
            appcan.window.evaluateScript('updateNews', "closeWindow('" + eventStatus + "')");
        } else {
