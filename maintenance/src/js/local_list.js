...
var wrapperObj = {
    ...
    bindEvent: function() {
        ...
        appcan.button(that.elem.upload, "btn-act", function() {
            if (that.isPostReady) {
                ...
                uexWindow.evaluateMultiPopoverScript('', 'local_list_content', 'unup_list', 'maintenanceList.uploadRecordsToService()');                
            }else{
        ...
        appcan.button(that.elem.unup_delete, "btn-act", function() {
           uexWindow.evaluateMultiPopoverScript('', 'local_list_content', 'unup_list', 'maintenanceList.deleteMaintance()');
        });
        /* 删除 trash */
        appcan.button(that.elem.trash_delete, "btn-act", function() {
            uexWindow.evaluateMultiPopoverScript('', 'local_list_content', 'trash_list', 'maintenanceList.deleteMaintance();');
        });
        appcan.window.on('resume', function() {
            
            uexWindow.evaluateMultiPopoverScript('', 'local_list_content', 'unup_list', 'maintenanceList.queryList()');
            uexWindow.evaluateMultiPopoverScript('', 'local_list_content', 'trash_list', 'maintenanceList.queryList()');
    ...
    bindCheckEvent: function() {
        ...
        this.unUp_allCheck.change(function() {
            ...
            uexWindow.evaluateMultiPopoverScript('', 'local_list_content', 'unup_list', 'maintenanceList.setCheckBoxIfChecked(' + bool + ')');

        });
        this.trash_allCheck.change(function() {
            var bool = $(this).prop('checked');
            uexWindow.evaluateMultiPopoverScript('', 'local_list_content', 'trash_list', 'maintenanceList.setCheckBoxIfChecked(' + bool + ')');
        });
    },
